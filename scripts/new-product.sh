...
find "$DIR" -type f \( -name '*.ts' -o -name '*.md' -o -name '*.json' -o -name '*.toml' -o -name '*.html' -o -name '*.js' -o -name '*.css' -o -name 'Makefile' -o -name 'Dockerfile' -o -name '.env.example' -o -name '*.yml' \) -not -path '*/vendor/*' -exec sed -i '' -e "s/__PRODUCT_SLUG__/$NAME/g" -e "s/__PRODUCT_TITLE__/$TITLE/g" {} +
